/**
 * 导表核心逻辑
 * 纯函数模块，不直接执行 IO，便于测试时注入 mock 依赖。
 */
 ...
 * @param {object} [options.fs] - 文件系统模块（默认使用 Node.js fs）
 * @param {object} [options.path] - 路径模块（默认使用 Node.js path）
 * @param {string} [options.tablesDir] - 配表源目录
 * @param {string} [options.outputTsDir] - TS 输出目录
 */
function exportTables(options = {}) {
  const fs = options.fs || require('fs');
  const path = options.path || require('path');
  const TABLES_DIR = options.tablesDir || path.join(__dirname, '..', 'data', 'tables');
  const OUTPUT_TS_DIR = options.outputTsDir || path.join(__dirname, '..', 'src', 'data');
  ...
  for (const file of files) {
    ...
    const { fieldDef, data, dataHeaders } = parseMarkdownContent(content);
